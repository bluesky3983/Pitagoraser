...
function Apply (dummyTarget : Transform, dummyCenter : Vector3)
{
	...
	// When pressing Fire3 (left cmd) the camera will snap to the target direction real quick.
...
function Update()
{
	if( Input.GetAxis("RightVertical") > 0 && distance < 3){
		distance += 0.05;	
	}
	else if( Input.GetAxis("RightVertical") < 0 && distance > 0.3600003){
		distance -= 0.05;
	}
	else{
	}
